...
function main() {
    ...
    var sites = {
        ...
        'll': {
            ...
            parse: function (url) {
                return /^https?:\/\/www\.liveleak\.com\/view.+i=([0-9a-z_]+)/i.exec(url) ? {
                    vid: RegExp.$1
                } : null;
            },
            sizes: [
            ...
            embed: function (vid) {
                return 'http://www.liveleak.com/e/' + vid + '?autostart=true';
            },
            ...
            request: function (vid, f) {
                net.text('http://www.liveleak.com/view?i=' + vid, [/Item not found!/i, /<title>LiveLeak\.com - (.+?)<\/title>/i, /"og:image" content="(.+?)"/], function (code, m) {
                    ...
                    if (notfound) {
                        ...
                        window.setTimeout(f, 0, {
                            title: title ? title[1] : 'Unknown video',
                            preview: preview ? preview[1].replace('_thumb_', '_sf_') : null
                        });
                    }
                });
            }
        ...
        'wh': {
            ...
            parse: function (url) {
                return /^https?:\/\/(www\.|m\.)?worldstarhiphop\.com\/.+v=([a-z0-9]+)/i.exec(url) ? {
                    vid: RegExp.$2
                } : null;
            },
            sizes: [
            ...
            embed: function (vid) {
                return 'http://www.worldstarhiphop.com/videos/ea/16711680/' + vid;
            },
        ...
        'dm': {
            ...
            parse: function (url) {
                return /^https?:\/\/www\.dailymotion\.com\/video\/([a-z0-9]+)(.+start=([0-9]+))?/i.exec(url) ? {
                    vid: RegExp.$1,
                    t: RegExp.$3
                } : null;
            },
            ...
            embed: function (vid, t) {
                return 'https://www.dailymotion.com/embed/video/' + vid + '?logo=0&autoplay=1' + (t ? '&start=' + t : '');
            },
            url: function (vid, t) {
                return 'https://www.dailymotion.com/video/' + vid + (t ? '?start=' + t : '');
            },
